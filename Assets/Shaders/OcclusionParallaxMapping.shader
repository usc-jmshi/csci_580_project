Shader "OcclusionParallaxMapping" {
  Properties {
  ...
  SubShader {
    ...
    Pass {
      ...
      half2 parallax_mapping(half2 tex_coord, float3 viewdir)
      {
        ...
        // interpolation for occlusion parallax mapping
        half2 prev_tex_coord = curr_tex_coord - delta_tex_coord;

        float after_height = current_layer_height - curr_height;
        float before_height = current_layer_height - layer_height - (SAMPLE_TEXTURE2D(_DepthMap, sampler_BaseMap, (half2)prev_tex_coord)).r;

        float weight = after_height / (after_height - before_height);
        half2 final_tex_coord = prev_tex_coord * weight + curr_tex_coord * (1.0 - weight);
