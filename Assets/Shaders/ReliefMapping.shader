Shader "ReliefMapping" {
  Properties {
    ...
    _BinarySearchDepth ("Binary Search Depth", Integer) = 6
  ...
  SubShader {
    ...
    Pass {
      ...
      half3 relief_mapping(half2 tex_coord, float3 viewdir)
      {
        ...
        float curr_layer_height = 0.0;
        float prev_layer_height = 0.0;
        ...
        for (float i = 0.0; i < num_layers; i += 1.0) {
          ...
          {
            prev_tex_coord = curr_tex_coord;
            prev_layer_height = curr_layer_height;
          }
        }
        bool reached_max_height = curr_layer_height == prev_layer_height;

        // if failed to find intersection, then return a negative tex coord (will show up as transparent)
        half2 half_tex_coord;
        float half_height;
        float half_layer_height;
        // binary search between this layer and the immediately preceding layer
        for (int j = 0; j < _BinarySearchDepth; j += 1) {
          half_tex_coord = 0.5 * (prev_tex_coord + curr_tex_coord);
          half_height = (SAMPLE_TEXTURE2D(_DepthMap, sampler_BaseMap, (half2)half_tex_coord)).r;
          half_layer_height = 0.5 * (prev_layer_height + curr_layer_height);
          if (half_layer_height >= half_height)
          {
            curr_tex_coord = half_tex_coord;
            curr_layer_height = half_layer_height;
          }
          else
          {
            prev_tex_coord = half_tex_coord;
            prev_layer_height = half_layer_height;
          }
        }

        return half3(half_tex_coord, reached_max_height ? -1 : 1);
      }
      ...
      half4 fragment(Varyings i): SV_TARGET {
        ...
        half3 new_tex_coord =  relief_mapping(i.uv, viewdir_ts);

        if (new_tex_coord.z < 0)
        {
          return saturate(half4(0.0,0.0,0.0,0.0));
        }
        else
        {
          // sample texture and normals based on new estimated texture coordinate
          half4 base_map_color = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, new_tex_coord.xy);
          half3 normal = UnpackNormal(SAMPLE_TEXTURE2D(_NormalMap, sampler_BaseMap, new_tex_coord.xy));
